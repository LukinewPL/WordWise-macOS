...
struct SetDetailView: View {
    ...
    @Bindable var set: WordSet; @Environment(\.modelContext) var ctx
    ...
    var body: some View {
        VStack {
            ...
            Picker(lm.t("translation"), selection: $set.translationDirectionRaw) { Text("PL → EN").tag(0); Text("EN → PL").tag(1) }
                .pickerStyle(SegmentedPickerStyle()).padding()
            List(set.words) { w in
                HStack { Text(w.polish); Spacer(); Text(w.english); if w.isMastered { Image(systemName: "star.fill").foregroundColor(.yellow) } }.listRowBackground(Color.clear)
