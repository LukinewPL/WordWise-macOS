        ...
        VStack(spacing: 15) {
            HStack {
                VStack(alignment: .leading) {
                    Text(set.name).font(.headline).foregroundColor(.white)
                    Text("\(set.words.count) \(lm.t("words"))").font(.subheadline).foregroundColor(.glassCyan)
                }
                Spacer()
            ...
            ProgressView(value: Double(set.words.filter(\.isMastered).count), total: max(1, Double(set.words.count)))
                .tint(.glassCyan)
